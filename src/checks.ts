 ...
 * @param path
 */
export function checkRequired (required: boolean, value: unknown, path: string): void {
...
export function checkSchemaAttributes (attributes: SchemaAttributes, path = ''): void {
  ...
  if (type != null) {
    if (type instanceof Array) {
      type.forEach((el) => {
        if (typeof el !== 'string') {
          throw new TypeError(`${joinPath(path, 'type')} is not valid`)
        }
      })
    } else if (typeof type !== 'string') {
      throw new TypeError(`${joinPath(path, 'type')} = "${type}" must be a string or an array of strings`)
    }
  }
  // Check enum
  if (typeof attributes.enum !== 'undefined' && !(attributes.enum instanceof Array)) {
    throw new TypeError(`${joinPath(path, 'enum')} must be an array`)
  }
  // Check format
  const { format } = attributes
  if (!['undefined', 'string'].includes(typeof format)) {
    throw new TypeError(`${joinPath(path, 'format')} must be a string`)
  }
  // Check items
  const { items } = attributes
  if (typeof items !== 'undefined' && typeof items !== 'object') {
    throw new TypeError(`${joinPath(path, 'items')} must be an object`)
  }
  // Check length
  if (!['undefined', 'number'].includes(typeof length)) {
    throw new TypeError(`${joinPath(path, 'length')} must be a number`)
  }
  // Check maximum value
  const { maximum } = attributes
  if (!['undefined', 'number'].includes(typeof maximum)) {
    throw new TypeError(`${joinPath(path, 'maximum')} must be a number`)
  }
  // Check max length
  const { maxLength } = attributes
  if (!['undefined', 'number'].includes(typeof maxLength)) {
    throw new TypeError(`${joinPath(path, 'maxLength')} must be a number`)
  }
  // Check minimum value
  const { minimum } = attributes
  if (!['undefined', 'number'].includes(typeof minimum)) {
    throw new TypeError(`${joinPath(path, 'minimum')} must be a number`)
  }
  // Check min length
  const { minLength } = attributes
  if (!['undefined', 'number'].includes(typeof minLength)) {
    throw new TypeError(`${joinPath(path, 'minLength')} must be a number`)
  }
  // Check pattern
  const { pattern } = attributes
  if (!['undefined', 'string'].includes(typeof pattern)) {
    throw new TypeError(`${joinPath(path, 'pattern')} must be a string`)
  }
  ...
  if (typeof patternProperties !== 'undefined' && (typeof patternProperties !== 'object' || patternProperties === null)) {
    throw new TypeError(`${joinPath(path, 'patternProperties')} must be an object`)
  }
  // Check properties
  const { properties } = attributes
  if (typeof properties !== 'undefined' && (typeof properties !== 'object' || properties === null)) {
    throw new TypeError(`${joinPath(path, 'properties')} must be an object`)
  }
  // Check title
  const { title } = attributes
  if (!['undefined', 'string'].includes(typeof title)) {
    throw new TypeError(`${joinPath(path, 'title')} must be a string`)
  }

  // Check denied values
  const { denied } = attributes
  if (typeof denied !== 'undefined' && !(denied instanceof Array)) {
    throw new TypeError(`${joinPath(path, 'denied')} must be an array`)
  }
  // Check max words
  const { maxWords } = attributes
  if (!['undefined', 'number'].includes(typeof maxWords)) {
    throw new TypeError(`${joinPath(path, 'maxWords')} must be a number`)
  }
  // Check min words
  const { minWords } = attributes
  if (!['undefined', 'number'].includes(typeof minWords)) {
    throw new TypeError(`${joinPath(path, 'minWords')} must be a number`)
  }
  // Check required
  const { required } = attributes
  if (!['undefined', 'boolean'].includes(typeof required)) {
    throw new TypeError(`${joinPath(path, 'required')} must be a boolean`)
  }
  ...
  if (attributes.enum && attributes.denied) {
    throw new TypeError(`${joinPath(path, 'enum')} and ${joinPath(path, 'denied')} cannot be defined together`)
  }
}
