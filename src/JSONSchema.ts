...
/**
 * Makes all properties partial. (required: false)
 */
export type PartialProperties<A extends SchemaAttributes> =
  Omit<A, 'properties'>
  & {
  properties: {
    [K in keyof A['properties']]: Omit<A['properties'][K], 'required'> & { required: false }
  }
}
...
/**
 * Makes all properties mandatory. (required: true)
 */
export type RequiredProperties<P extends SchemaAttributes> =
  Omit<P, 'properties'>
  & {
  properties: {
    [K in keyof P['properties']]: Omit<P['properties'][K], 'required'> & { required: true }
  }
}
...
class JSONSchema<A extends SchemaAttributes> {
  ...
  partial (): JSONSchema<PartialProperties<A>> {
    const attributes = this.toJSON() as unknown as PartialProperties<A>

    if (attributes.properties) {
      for (const key in attributes.properties) {
        attributes.properties[key].required = false
      }
    }
  ...
  required (): JSONSchema<RequiredProperties<A>> {
    const attributes = this.toJSON() as unknown as RequiredProperties<A>

    if (attributes.properties) {
      for (const key in attributes.properties) {
        attributes.properties[key].required = true
      }
    }
    return new JSONSchema(attributes)
  }
